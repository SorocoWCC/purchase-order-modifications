(function() {
    ...
    var body;
    window.SOROCOModel = {
        modulesInstances: [], scope: {}, inited: false, currentModel: '', odooM: {},
        _attachHandlers: function(){
            ...
            jQuery(document).ready(function(){
                ...
                jQuery('nav ul li a.oe_menu_toggler, ul#menu_more li a').on('click', function(){
                    ...
                    window.SOROCOModel._initModulesRefresh();
                });
            });
        },
        _modelWatcher: function(scope){
            console.log('-> Waiting for Odoo model to load');
            var srcModel = this,
        ...
        _markupWatcher: function(){
            console.log('-> Looking for custom module files');
            var srcModel = this,
            counter = 1,
            markupWatcher = window.setInterval(function() {
                var view;

                if (srcModel.modulesInstances.length > 0) {
                    jQuery.each(srcModel.modulesInstances, function(index, module) {
                        var cont = jQuery(module.container);

                        if (cont.length > 0) {
                            srcModel.scope.customModuleSelector = cont;
                            console.log('-> Module Found');
                            console.log('-> initializing module: '+ srcModel.scope.controllerId);
                            module.inited = true;
                    ...
                    if (counter >= 4) {
                        console.log('No custom files found for this module');
                        window.clearInterval(markupWatcher);
                    }
                    counter ++;
                }else{
                    console.log('There are no custom modules on the queue');
                    window.clearInterval(markupWatcher);
                }
            }, 1500);
        },
        _initModulesRefresh: function(){
            var srcModel = this, found = false;
            console.log('->Looking for current instantiate module');
            jQuery.each(srcModel.modulesInstances, function(index, module) {
                if (module.inited) {
                    console.log('-> Current Module found');
                    found = true;
                    module.instance.suspend();
                    module.inited = false;
                    console.log('-> Instance down');
                }
            });
            if (!found) {
                console.log('Unable to found a module instance running');
            }
            srcModel._modelWatcher(srcModel.odooM);
        },
        ...
        initModule: function(module, moduleContainer) {
            this.modulesInstances.push({instance: module, container: moduleContainer, inited: false});
        },
        init: function(scope){
            this._initGlobalDepencies();
